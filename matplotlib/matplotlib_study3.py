...
#         continue
#     label = ann['label']
#     pos = (label['x'], label['y'], label['width'], label['height'])
# print(pos)
...
for ann in annotations:
    ...
    if 'vehicle' != code:
        continue
    label = ann['label']
    pos.append((label['x'], label['y'], label['width'], label['height'], colors[idx]))
    idx += 1
print(pos)
...
for(x,y,width,height, colors) in pos:
# 박스 좌표 설정
# x, y, width, height = pos # 언패킹으로 넣어준다.
...
# 사각형 박스 만들기
    box = patches.Rectangle(
        ...
        edgecolor=colors, # 박스 테두리 색
        linewidth=2 # 테두리 선 굵기
    )
    ax.add_patch(box)
    # 바운딩 박스위에 텍스트 넣기
    plt.text(x,y-10, 'object', color='red',
             fontsize=10, bbox=dict(facecolor='white', edgecolor='red', pad=2))
