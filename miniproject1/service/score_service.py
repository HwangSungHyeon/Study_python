...
def create_score_service(score, db):
    scoreData = [score.python_score, score.numpy_score, score.pandas_score,
                score.java_score, score.project_score]
    ...
    for i, v in enumerate(scoreData):
        if v < 0 or v>100:
            raise HTTPException(
                status_code=404,
                message=subject[i] + "점수는 0이상 100 이하만 가능합니다."
            )
...
def get_score_list(db):
    score_list = db.query(Score, Student).join(Student, Score.student_id == Student.id).all()

    result = []
    for score, std in score_list:
        total_score = (score.python_score + score.numpy_score + score.pandas_score +
                    score.java_score + score.project_score)
