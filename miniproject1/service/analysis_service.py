...
def create_report_service(db):
    ...
    story = []
    title = Paragraph("학생 학습 분석리포트", style["Title"])
    story.append(title)

    summary_data = get_class_summary(db)
    story.append( Paragraph("1. 반 전체 통게", style["Heading1"]))
    story.append(Spacer(1,10))
    story.append(Paragraph(f"전체 학생 수: {summary_data['total_student']}명",
                            style["BodyText"]
                            ))
    story.append(Paragraph(f"반 평균 점수: {summary_data['avg_score']}점",
                            style["BodyText"]
                            ))
    story.append(Paragraph(f"평균 출석률: {summary_data['avg_attend_rate']}%",
                            style["BodyText"]
                            ))
    ...
    story.append(Spacer(1,10)) # Spacer-pdf 줄바꿈 및 여백 (몇줄, 아래쪽여백 얼마(pt))
    story.append(
        Paragraph(
...
def get_graph_summary_service(db):
    ...
    attend_rates=[]
    for score in scores:
        avg = (score.python_score + score.numpy_score + score.pandas_score + score.java_score + score.project_score) / 5
...
def score_graph_make(db):
    ...
    labels=["0~50점", "50점~60점", "60점~70점", "70점~80점", "80점~90점", "90점~100점"]
...
def attend_graph_make(db):
    ...
    ax.set_title("출석률 분표", fontsize=16, fontweight="bold", pad=20)
...
def get_student_status_service(db):
    ...
    for std in students:
        score = db.query(Score).filter(Score.student_id == std.id).first()
        attend = db.query(Attend).filter(Attend.student_id == std.id).first()
...
def get_class_summary(db):
    ...
    for score in scores:
        avg = (
            score.python_score + score.numpy_score + score.pandas_score + score.java_score + score.project_score
        ) / 5
        temp_avg_score += avg
